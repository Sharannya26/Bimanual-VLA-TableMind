...
def run_script(script_name: str) -> tuple[int, str, str]:
    ...
    UTF-8 is explicitly selected because the TABLEMIND demos
    contain Unicode characters such as arrows, checkmarks,
    and fire emojis.
    """
...
class PresentationCamera:
    ...
    This camera is deliberately separate from TABLEMIND's calibrated
    table_overview perception camera so vision/YOLO/OpenVINO keeps using
    the original calibrated view.
    """
...
def run_live_bidirectional_goal_demo() -> tuple[bool, object | None, object | None, str, str, str]:
    """
    Run the proven Milestone 9.5 demo IN THIS STREAMLIT PROCESS while
    capturing the exact MuJoCo simulation instance created by the demo.

    This avoids launching a second terminal process and lets the UI render
    the actual MuJoCo world used by demo_bidirectional_goal.py.
    The 9.5 robotics implementation itself is not changed.
    """
    ...
    # Keep the original descriptor so the class is restored exactly after
    # the demo. Assigning a bound method back onto the class would be wrong.
    original_create_descriptor = BimanualTableSimulation.__dict__["create"]
    original_create = BimanualTableSimulation.create
    ...
    def captured_create(cls):
        ...
        try:
            ...
            simulation_holder["before_frame"] = camera.capture()
        finally:
    ...
    try:
        ...
        before_frame = simulation_holder["before_frame"]
        after_frame = None
        ...
        if success:
            message = (
                "1 → 2 → 1 VERIFIED — the proven Milestone 9.5 "
                "goal-reconciliation demo ran inside the same MuJoCo "
                "process and the final physical state was rendered in the UI."
            )
        else:
            message = (
                "The Milestone 9.5 demo completed without its success "
                "marker. Open the diagnostic trace below."
            )
        ...
        BimanualTableSimulation.create = original_create_descriptor


def run_live_mujoco_demo(
    instruction: str,
) -> tuple[bool, object | None, object | None, str]:
    """
...
if "instruction" not in st.session_state:
    st.session_state.instruction = "Set the table for two."
...
if st.session_state.mode == "execution":
    ...
    st.markdown("### TABLEMIND / LIVE MUJOCO EXECUTION")
    st.caption("Physical AI pipeline active")
    ...
    execution_steps = [
        ("01", "LANGUAGE", "Natural instruction"),
        ("02", "VISION", "Scene grounding"),
        ("03", "REASONING", "Goal understanding"),
        ("04", "PLANNING", "Task decomposition"),
        ("05", "BIMANUAL", "Dual-arm coordination"),
        ("06", "VERIFY", "Physical validation"),
    ]
...
with command_left:
    ...
    with st.container(
        ...
        execute_column, goal_column = st.columns(
            2,
            gap="small",
        )
        ...
        with execute_column:
            ...
            if st.button(
                ...
                st.session_state.mode = "execution"
        ...
        with goal_column:
            ...
            if st.button(
                ...
                with st.status(
                    ...
                    st.write("◉ Initial physical goal → 1 guest")
                    st.write("◉ User modification → 2 guests")
                    st.write("◉ Physical addition → plate + glass")
                    st.write("◉ Verification → 2 complete place settings")
                    st.write("◉ User modification → 1 guest")
                    st.write("◉ Physical removal → setting #2")
                    st.write("◉ Final verification → 1 complete place setting")
                ...
                if before_frame is not None:
                    st.markdown("### INITIAL PHYSICAL STATE")
                    st.image(
                        before_frame,
                        use_container_width=True,
                    )
                ...
                if after_frame is not None:
                    st.markdown("### FINAL PHYSICAL STATE")
                    st.image(
                        after_frame,
                        use_container_width=True,
                    )
                ...
                with st.expander("9.5 goal-reconciliation trace"):
